function [c] = compute_planner_vf(eq, param, glob, options)
%% Unpack
Phi         = glob.Phi;
Emat        = glob.Emat;
Kp          = eq.Kp;
...
U               = glob.lambda * u_b + (1 - glob.lambda) * u_w - ((-mu) - (-mup)) .* B .* u_w_c + (-mup) .* (-c_w .* u_w_c - L .* u_w_l);
jac             = [Phi,     -glob.beta_w * Phi_KBmuZp;
                   -Emat,                       Phi];
c               = jac \ [U; zeros(size(glob.s, 1), 1)];
